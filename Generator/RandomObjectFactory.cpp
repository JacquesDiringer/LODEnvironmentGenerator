...
namespace Generator
{
	...
	void RandomObjectFactory::GenerateLevel(shared_ptr<Item> parent, int childrenNumber, const Math::Matrix4& futureTransformation, const Math::Matrix4& worldMatrix, vector<shared_ptr<Item>>* itemVector)
	{
		// TODO: Find a better way to a generate random number.
		std::srand(std::hash<Math::Matrix4>()(worldMatrix));
